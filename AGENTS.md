...
This repository targets Node.js 18+ on macOS/Linux/Windows. Always install dependencies via `npm install` (or `npm ci`) before running scripts so lockfile stays in sync.
...
When extending or debugging, run the local dev server with `npm run dev` and verify your change in the browser at http://localhost:3000.

For formatting/linting use `npm run lint` and for building use `npm run build` to catch TypeScript/Next errors early.

Avoid editing anything under `node_modules/`. If you need new dependencies, add them with `npm install <pkg> --save` (respecting the lockfile) and mention the reason in your notes.
