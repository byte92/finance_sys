...
StockTracker 是一个纯前端的开源股票交易记录与盈亏分析工具。MVP 以 **A股** 为核心，未来将逐步扩展支持港股、美股、基金、加密货币等资产类型。

数据默认存储在浏览器本地，支持 JSON 格式一键导出备份，设计上充分考虑了未来迁移云端数据库的可扩展性。
...
| 存储 | LocalStorage（via Zustand persist） | 零后端依赖，可离线使用 |
