...
> 当前实现状态：Agent Runtime、Skill Registry、固定分析 Task、Trace 持久化、`security.resolve` 基础标的解析、`web.search` / `web.fetch` 公开网页补数、加密资产候选解析和服务端日志均已落地。本文档同时保留一部分路线图内容，用于说明后续演进方向。
