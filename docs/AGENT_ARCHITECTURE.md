...
> 当前实现状态：Agent Runtime、Skill Registry、固定分析 Task、Trace 持久化、`security.resolve` 基础标的解析、`web.search` / `web.fetch` 公开网页补数、加密资产候选解析和服务端日志均已落地。V2 的链式 Skill 执行、LLM 对话内澄清、运行时 scope 校验和上下文压缩也已落地。本文档同时保留一部分路线图内容，用于说明后续演进方向。

## 2. 设计原则

- **领域优先**：Agent 只服务股票、持仓、交易、风险、行情、估值和资产配置，不做通用助理。
- **按需取数**：默认不注入全量持仓，只有问题需要的数据才进入上下文。
- **模型规划，确定性补位**：Planner 让模型负责意图、市场和 Skill 编排；证券名称/代码转换统一通过 `security.resolve` 等基础 Skill 补齐，避免在 Planner 中堆硬编码。
...
lib/agent/
  ...
  planner.ts
    意图识别、实体识别、数据需求规划。模型负责计划生成，确定性逻辑负责安全边界和候选补位。
...
| Skill | 数据范围 | 用途 |
| --- | --- | --- |
| `security.resolve` | `stock.read` / `quote.read` | 基础标的实体解析：将名称、代码、简称或自然语言问题解析为标准 code、name、market 和持仓状态。 |
...
| `stock.getExternalQuote` | `quote.read` | 获取未持仓标的行情和估值，不把外部标的当作持仓事实。 |
| `stock.getTechnicalSnapshot` | `quote.read` | 获取 K 线衍生技术指标摘要。 |
| `stock.getFinancials` | `quote.read` / `network.fetch` | 获取财报关键数据；结构化数据不足时给出后续公开搜索建议。 |
...
| `market.resolveCandidate` | `quote.read` | 旧版兼容解析入口；新计划应优先使用 `security.resolve`。 |
| `web.search` | `network.fetch` | 检索最新公开网页来源；个股新闻/公告、利好利空、A 股大盘政策或盘面新闻会由 Planner 触发，A 股公告优先扩展到巨潮资讯和交易所站点。 |
| `web.fetch` | `network.fetch` | 对白名单金融域名发起受控抓取，作为财报、公告、新闻等公开数据兜底。 |
...
Planner 不再把每个业务场景都写成固定关键词规则。它的主要职责是让模型从用户问题中抽取：

- 意图：单标的、组合、大盘、交易复盘、公开网页补数或拒答。
- 实体：用户原文中的名称、代码、市场线索和时间条件。
- Skill 编排：需要调用哪些基础 Skill，以及每个 Skill 的参数和原因。
- 搜索上下文：当需要 `web.search` 时，由模型提供 query、候选 queries 和 sourceHints，而不是在代码里按场景硬编码改写搜索词。

确定性逻辑仍然存在，但定位为安全和质量补位：

- 明显越界问题可以快速拒答。
- 明确代码形态可以作为低成本候选。
- 本地持仓匹配和外部候选统一通过 `security.resolve` 执行。
- 多候选或低置信结果由对话澄清，而不是在路由层拦截。

### 7.2 基础解析 Skill

名称、代码、简称、ETF 名称和自然语言标的提取统一进入 `security.resolve`。该 Skill 的数据来源顺序：
  ...
  "requiredSkills": [
    {
      ...
      "args": { "query": "成都银行", "limit": 5 },
      "reason": "用户询问单只股票，需要先解析名称和市场"
    }
  ],
  "responseMode": "answer"
}
...
- 更完善的澄清机制。
- 当前实现已支持最多 10 轮链式 Skill、LLM 解析澄清回复、无法识别时继续用对话追问、Skill scope 校验和超大 SkillResult 压缩。
