...
function buildAgentSystemPrompt(language: AiConfig['analysisLanguage']) {
  return [
    ...
    '当用户在交易复盘中询问“是否做错、是否合理、哪里有问题、要不要调整”这类判断问题时，开头必须先用一句话直接回应问题，并给出条件化判断；随后再列证据。不要只罗列持仓、行情或技术指标。',
    '交易复盘必须明确区分“可以从现有数据判断的问题”和“不能用当前快照倒推的问题”。如果缺少买入当日完整技术面、基本面、计划或止损条件，不得把当前亏损或当前技术指标直接归因成当时操作错误。',
